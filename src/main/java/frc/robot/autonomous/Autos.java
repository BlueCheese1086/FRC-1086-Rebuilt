...
public class Autos {
    ...
    public static void setup(Drive kDrive, Intake kIntake) {
        ...
        drive = kDrive;
        intake = kIntake;
        factory = new AutoFactory(drive::getPose, drive::setPose, run(), DriverStation.getAlliance().orElse(Alliance.Blue).equals(Alliance.Red),drive);
    }
    public static Command runAutonomous(String path) {
        ...
        traj.atTime("IntakeRun").onTrue(intakeRun().onlyIf(() -> (runSubsystems.get() == 1)).until(traj.atTime("IntakeStop")::getAsBoolean));
