...
public class LauncherCalculator {
    ...
    public static LauncherCalculator getInstance() {
        if (instance == null)
            instance = new LauncherCalculator();
        return instance;
    }
    ...
    public LaunchingParameters getParameters(Supplier<Pose2d> drivePose, Supplier<ChassisSpeeds> roboRelativeSpeed) {
        boolean passing = AllianceFlipUtil.applyX(drivePose.get().getX()) > FieldConstants.LinesVertical.hubCenter;
        ...
        var robotVelocity = ChassisSpeeds.fromRobotRelativeSpeeds(robotRelativeVelocity, drivePose.get().getRotation());
        var robotAngle = drivePose.get().getRotation();
        ChassisSpeeds launcherVelocity = transformVelocity(
                robotVelocity, ShooterConstants.Mechanical.shooterPose.getTranslation().toTranslation2d(), robotAngle);
        ...
        Pose2d lookaheadRobotPose = lookaheadPose.transformBy(new Transform2d(ShooterConstants.Mechanical.shooterPose.getTranslation().toTranslation2d(), estimatedPose.getRotation()));
        ...
        double flywheelVelocity = passing
        ...
        Rotation2d hubAngle = new Rotation2d(
                Math.asin(
                        MathUtil.clamp(
                                ...
                                -1.0,
                                1.0)));
        Rotation2d driveAngle = fieldToHubAngle.plus(hubAngle).plus(ShooterConstants.Mechanical.shooterPose.getRotation().toRotation2d());
        return driveAngle;
    }
     ...
     */
    public static Pose2d getStationaryAimedPose(Translation2d robotTranslation, Rotation2d robotRotation, boolean forceBlue) {
        // Calculate target
        ...
        return new Pose2d(
                robotTranslation, getDriveAngleWithLauncherOffset(new Pose2d(robotTranslation, robotRotation), target));
    }
