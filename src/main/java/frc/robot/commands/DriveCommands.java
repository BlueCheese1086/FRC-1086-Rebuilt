  ...
  public static Command moveBack(Drive drive) {
    ...
    return Commands.run(
        () -> {
          ...
          drive.runVelocity(ChassisSpeeds.fromFieldRelativeSpeeds(new ChassisSpeeds(speed, 0.0, 0.0), drive.getRotation()));
        },
        drive)
        .beforeStarting(
            () -> {
    ...
    public static Command joystickDriveLockRadiusToTarget(
      ...
      return Commands.run(
              () -> {
                ...
                Logger.recordOutput("Lock Radius/Speeds", ChassisSpeeds.fromFieldRelativeSpeeds(
                        speeds,
                        isFlipped
                            ? drive.getRotation().plus(new Rotation2d(Math.PI))
                            : drive.getRotation()));
