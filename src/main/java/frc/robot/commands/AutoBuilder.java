...
public class AutoBuilder {
  ...
  public Command build() {
    ...
    return AutoRoutines.runPath(
        ...
        .andThen(
            startShoot
                ? // shoot and go to intake if preloaded
                dummyShoot().andThen(follow(pShoot + "-" + getIntakePoint()))
                : Commands.none(),
            intake.equals("none")
                ? // if no intake was selected,
                follow(nzEntry.getSelected() + "-" + nzTarget.getSelected())
                    .andThen( // go to neutral zone target
                        dummyIntake(), // intake
            ...
            !climb.equals("none")
                ? // if climb selected,
                follow(fShoot + "-" + climb).andThen(dummyClimb())
                : // go to climb positon and climb
                Commands.none());
  }
  ...
  private String getIntakePoint() { // get intake point based on intake type (either neutral zone or
    // depot/outpost)
    return intakeType.getSelected().equals("none")
        ? nzEntry.getSelected()
        : intakeType.getSelected();
  }
